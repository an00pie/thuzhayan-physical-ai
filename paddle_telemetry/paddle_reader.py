...
class TrafficLightHardware:
    """Controls physical Red, Yellow, and Green breadboard LEDs via Raspberry Pi 5 GPIO pins."""

    def __init__(self, green_pin: int = 22, yellow_pin: int = 27, red_pin: int = 17, enabled: bool = True):
        ...
        if self.enabled:
            try:
                ...
                print(f"--> RPi 5 Hardware GPIO LEDs Active: Green(GPIO {green_pin}), Yellow(GPIO {yellow_pin}), Red(GPIO {red_pin})")
            except Exception as err:
                print(f"--> RPi 5 Hardware GPIO LEDs not attached ({err}). Software/Web mode active.")
                self.enabled = False

    def update(self, light_status: str) -> None:
        """Update physical breadboard LEDs according to light_status ('GREEN', 'YELLOW', 'RED')."""
        if not self.enabled:
            return

        try:
            if light_status == "GREEN":
                if self.green_led: self.green_led.on()
                if self.yellow_led: self.yellow_led.off()
                if self.red_led: self.red_led.off()
            elif light_status == "YELLOW":
                if self.green_led: self.green_led.off()
                if self.yellow_led: self.yellow_led.on()
                if self.red_led: self.red_led.off()
            elif light_status == "RED":
                if self.green_led: self.green_led.off()
                if self.yellow_led: self.yellow_led.off()
                if self.red_led: self.red_led.on()
        except Exception:
            pass
...
def main() -> None:
    ...
    parser.add_argument("--green-pin", type=int, default=22, help="RPi 5 GPIO pin for Green breadboard LED (Default: 22 / Pin 15)")
    parser.add_argument("--yellow-pin", type=int, default=27, help="RPi 5 GPIO pin for Yellow breadboard LED (Default: 27 / Pin 13)")
    parser.add_argument("--red-pin", type=int, default=17, help="RPi 5 GPIO pin for Red breadboard LED (Default: 17 / Pin 11)")
    parser.add_argument("--disable-gpio-leds", action="store_true", help="Disable physical GPIO breadboard LEDs")
    args = parser.parse_args()

    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)

    traffic_hw = TrafficLightHardware(
        green_pin=args.green_pin,
        yellow_pin=args.yellow_pin,
        red_pin=args.red_pin,
        enabled=not getattr(args, 'disable_gpio_leds', False)
    )
        ...
        with output.open("a", encoding="utf-8") as log:
            while args.seconds == 0 or (time.monotonic() - started) < args.seconds:
                if args.simulate:
                    ...
                    traffic_hw.update(light_status)
