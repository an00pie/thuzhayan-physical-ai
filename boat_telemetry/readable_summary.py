...
def print_formatted_summary(boat: dict | None, paddle: dict | None, compact: bool = False) -> None:
    ...
    print("")
    print(f"👥 DUAL-PADDLER SYNCHRONIZATION & TRAFFIC LIGHT STATUS:")
    light_status = paddle.get("sync_light_status", "GREEN" if sync_pct >= 85 else ("YELLOW" if sync_pct >= 70 else "RED"))
    light_emoji = "🟢" if light_status == "GREEN" else ("🟡" if light_status == "YELLOW" else "🔴")
    print(f"   ├─ Sync Light Indicator: {light_emoji} {light_status} ({sync_pct:.1f}% Sync Score)")
